    ...
    const fetchSellerStatus = async () => {
        ...
        try {
            ...
            if (['approved', 'rejected', 'pending'].includes(data.status)) { 
                ...
                switch (data.status) {
                    case 'approved':
                        setMessage('Your store has been approved, you can now add products to your store from your dashboard')
                        setTimeout(() => {
                            router.push('/store')
                        }, 5000)
                        break
                    case 'rejected':
                        ...
                        break
                    case 'pending':
                        setMessage('Your store request is pending, please wait for the admin to review your request')
                        break
                    default:
                        break;
                }
                ...
                <div className="mx-6 min-h-[70vh] my-16">
                    <form onSubmit={e => toast.promise(onSubmitHandler(e), { loading: "Submitting data..." })} className="max-w-7xl mx-auto flex flex-col items-start gap-3 text-slate-500">
                        ...
                        <label className="mt-10 cursor-pointer">
                            ...
                            <input type="file" accept="image/*" onChange={(e) => setStoreInfo({ ...storeInfo, image: e.target.files?.[0] ?? null })} hidden />
                        </label>
